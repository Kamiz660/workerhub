...
- **Mobile How to Use Strip**: Horizontal step indicator located below the job selection categories icon grid on mobile. Features raw icons (h-6 w-6) directly without background circles/boxes, arrow (`ArrowRight`, h-4 w-4) connectors with increased gray-400 contrast in between steps, slightly enhanced gray-500 font contrast text labels spaced with `mt-3`, and adjusted spacing (mt-10 mb-4).
